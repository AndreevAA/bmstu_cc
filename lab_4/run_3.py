...
class Parser:
    def __init__(self, input):
        ...
        self.output_queue = []  # Инициализация очереди вывода для ОПН
        self.operator_stack = []  # Инициализация стека операторов для ОПН
    ...
    # Метод для преобразования выражения в ОПН
    def expression_to_rpn(self, expression):
        # Удаление пробелов и разбиение выражения на токены
        ...
        for token in tokens:
            if self.__is_number(token):
                ...
                while self.operator_stack and self.operator_stack[-1] != '(':
                    self.output_queue.append(self.operator_stack.pop())
                self.operator_stack.pop()  # Удаление '('
...
def main(input_string):
    ...
    # Убедитесь, что входная строка является одной строкой без разделения на токены
    parser = Parser(input_string_split)
    parser.render_tree()
    # Вывод ОПН выражения
    # Используйте весь входной текст, а не узел дерева
    rpn_expression = parser.expression_to_rpn(input_string)
    print('ОПН выражение:', rpn_expression)
# Тестовый пример
test_input = """
{
    a + b + c * d - e
