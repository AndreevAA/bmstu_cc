...
class Parser:
    ...
    def to_postfix(self, root):
        ...
        def traverse(node):
            nonlocal postfix, stack

            if node.token_type in ['id', 'num']:
                postfix += node.token_type + " "
            else:
                while stack and self.precedence(stack[-1]) >= self.precedence(node.token_type):
                    postfix += stack.pop() + " "
                stack.append(node.token_type)
