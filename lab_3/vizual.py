...
class Parser:
    ...
    def create_new_node(self, parent_id, id, value):
        tmp_node = Node(parent_id = parent_id, id=id, value=value)
        self.node_lst.append(tmp_node)
        return tmp_node

    def __block(self, parent_node):
        block_node = self.create_new_node(parent_id = parent_node.id, id=self.__get_new_node_id(), value='block')
    ...
    def __operators_list(self, parent_node):
        __operators_list_node = Node(parent_id = parent_node.id, id=self.__get_new_node_id(), value='operators_list')
    ...
    def __operator(self, parent_node):
        ...
        if self.__get_current_token().isalpha():
            operator_node_value = Node(parent_id = operator_node.id, id=self.__get_new_node_id(), value=self.__get_current_token())
    ...
    def __factor(self, parent_node):
        ...
        if self.__get_current_token() in ('abs', 'not'):
            factor_node_value = Node(parent_id = factor_node.id, id=self.__get_new_node_id(), value=self.__get_current_token())
    ...
    def __primary(self, parent_node):
        ...
        if self.__get_current_token() == 'p' or self.__get_current_token().isdigit():
            primary_node_value = Node(parent_id = primary_node.id, id=self.__get_new_node_id(), value=self.__get_current_token())
    ...
    def __expression(self, parent_node):
        ...
        if self.__relation(expression_node):
            ...
            if self.__get_current_token() in ('and', 'or', 'xor'):
                expression_node_value = Node(parent_id = expression_node.id, id=self.__get_new_node_id(), value=self.__get_current_token())
    ...
    def __relation(self, parent_node):
        ...
        if self.__simple_expression(relation_node):
            if self.__get_current_token() in ('<', '<=', '==', '/>', '>=', '>'):
                relation_node_value = Node(parent_id = relation_node.id, id=self.__get_new_node_id(), value=self.__get_current_token())
    ...
    def __term(self, parent_node):
        ...
        if self.__factor(term_node):
            while self.__get_current_token() in ('*', '/', 'mod', 'rem'):
                term_node_value = Node(parent_id = term_node.id, id=self.__get_new_node_id(), value=self.__get_current_token())
    ...
    def __simple_expression(self, parent_node):
        ...
        if self.__get_current_token() in ('-', '+'):
            simple_expression_node_value = Node(parent_id = simple_expression_node.id, id=self.__get_new_node_id(), value=self.__get_current_token())
