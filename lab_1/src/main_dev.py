...
def epsilon_closure(nfa, states):
    ...
    while stack:
        state = stack.pop()
        if state in nfa and 'ε' in nfa[state]:  # Проверяем наличие ключа state в словаре nfa
...
def build_dfa(nfa):
    ...
    start_state = tuple(epsilon_closure(nfa, {0})) if 0 in nfa else ()  # Проверяем наличие узла 0 в НКА
    dfa[start_state] = {}
